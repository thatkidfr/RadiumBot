...
let restorePositionBind = "r"; // Bind to restore position
...
if (localStorage.getItem("depth") == null){
  ...
  restorePositionBind = localStorage.getItem("restorePosition");
...
function saveSettings(){
  ...
  localStorage.setItem("restorePosition", restorePositionBind);
...
function displaySettings(){
  ...
  document.getElementById("bot-currentResetBind").innerHTML = restorePositionBind;
...
//favicon.ico to be changed soon
overlay.innerHTML = `
<h1 id='bot-title'>Radium Bot</h1>
<img src='/favicon.ico' style="height:75px; width:auto;">
...
<div class='bot-rowContainer'><button id='bot-resetPosition'>reset position</button><p>Set keybind</p><p>(<p id='bot-currentResetBind'>r</p>)</p></div>
...
let BTNbotResetBind = document.getElementById("bot-resetPosition");
BTNbotResetBind.addEventListener('click', function() {
  restorePositionBind = prompt("Set bind to manually reset the position");
  saveSettings();
  displaySettings();
});
...
function generatePGN(){
  function getMovePlayed(obj){
    if (obj.children.length == 0){
        return obj.innerHTML.trim();
    } else {
        return obj.children[0].dataset.figurine.trim() + obj.innerText.trim();
    }
  }
  
  let movesElements = document.getElementsByClassName("offset-for-annotation-icon");
  let pgnText = "";
  
  for (let x = 0; x < movesElements.length; x++){
    if (x % 2 == 0){
      pgnText += (x/2 + 1).toString() + ". ";
    }
    pgnText += getMovePlayed(movesElements[x]) + " ";
  }
...
async function showBestMove(){
  ...
  drawArrow(bestMoves[0].move.slice(0,2), bestMoves[0].move.slice(2,4), 'rgba(0, 221, 255, 0.7)'); // Draw best arrow with an aqua color

  drawArrow(bestMoves[0].ponder.slice(0,2), bestMoves[0].ponder.slice(2,4), 'rgba(255, 55, 0, 0.7)'); // Draw opponents best response to the best move
  
  if (Math.abs(bestMoves[0].eval-bestMoves[1].eval)<evalDiffArrow){
    drawArrow(bestMoves[1].move.slice(0,2), bestMoves[1].move.slice(2,4), 'rgba(57, 143, 116, 0.7)'); // Second and third best moves shown if the eval difference between the move and the best move < evalDiffArrow variable
  }
  if (Math.abs(bestMoves[0].eval-bestMoves[2].eval)<evalDiffArrow){
    drawArrow(bestMoves[2].move.slice(0,2), bestMoves[2].move.slice(2,4), 'rgba(57, 143, 116, 0.7)');
  }
  //rgba(57, 143, 116, 0.7)
...
function getTop3MovesWithEval(worker, fen, depth = 15) {
    ...
    worker.onmessage = (e) => {
      ...
      if (line.startsWith("info") && line.includes("multipv")) {
        ...
        if (multipvMatch && pvMatch) {
          ...
          const move = moves[0];
          const ponder = moves[1] ?? null; // opponent's best response
          best.set(rank, { move, eval: evalScore, ponder });
        }
      }
...
function drawArrow(from, to, clr) { // Credit for this function goes to majid3612
...
if (moveList != null){
  ...
  errorMsg("Game not found - waiting...");
}
const bodyObserver = new MutationObserver(() => {
  const boardParent = document.querySelector('.move-list.chessboard-pkg-move-list-component');
  if (boardParent) {
    observer.observe(boardParent, { childList: true, subtree: true });
  }
});
...
document.addEventListener("keydown", function(event) {
  if (event.key == restorePositionBind){ // Attempt to manually restore the position by manually setting the players turn
    cachedMove = null;
    previousCount = getLength();
    playMove();
